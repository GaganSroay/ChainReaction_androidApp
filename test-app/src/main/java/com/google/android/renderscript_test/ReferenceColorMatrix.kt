...
fun referenceColorMatrix(
    ...
    output.forEach(restriction) { x, y ->
        ...
        val inFloatValue =
            FloatArray(4) { if (it >= inputVectorSize) 0f else byteToUnitFloat(inUByteValue[it]) }
        val outFloatValue = multiplyAndAdd(matrix, inFloatValue, addVector)
        val outUByteValue =
            UByteArray(paddedSize(output.vectorSize)) { unitFloatClampedToUByte(outFloatValue[it]) }
        output[x, y] = outUByteValue
    }
    return outputArray
}

private fun multiplyAndAdd(
    matrix: FloatArray,
    inVector: FloatArray,
